class TTeamInfoesController < ApplicationController
  ...
  def team_student_info
      ...
      @team_student_ck = TUserInfo.student_all.joins(:t_station_info, :t_record_infoes).where('t_station_info.F_duan_uuid = ?', TDuanInfo.find_by(F_name: params[:name]).F_uuid).select('t_user_info.F_id,t_station_info.F_name').distinct.group('t_team_info.F_name').size
  ...
  def team_score_info
      ...
      @team_60_bellow_scores = TTeamInfo.where('t_team_info.F_station_uuid = ?', TDuanInfo.find_by(F_name: params[:name]).F_uuid).joins(t_user_infoes: :t_record_infoes).where('t_record_info.F_score < ?', 60).group('t_team_info.F_name').size
