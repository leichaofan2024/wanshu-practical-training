class ApplicationRecord < ActiveRecord::Base
  ...
  scope :student_all , -> (begin_time,end_time){where.not("t_user_info.F_station_uuid": StationEquipmentMaintain.equipment_maintain(begin_time,end_time)).where("t_user_info.F_type=? AND t_user_info.status =?",0,"在职").where.not("t_user_info.F_uuid": TVacationInfo.student_transfer(begin_time,end_time)).where.not("t_user_info.F_id": TVacationInfo.student_long_vacation(begin_time,end_time))}
