module EmployeesHelper
  ...
  def employe_user(duan)
     @user = TUserInfo.student_all(Time.now.beginning_of_month, Time.now.end_of_month).joins(:t_duan_info).where("t_duan_info.F_name = ? ", duan).pluck("t_user_info.F_id").uniq.size
  ...
  def employe_record(duan)
    @user = TUserInfo.student_all(Time.now.beginning_of_month, Time.now.end_of_month).joins(:t_duan_info, :t_record_infoes).where("t_duan_info.F_name = ? ", duan).where("t_record_info.F_time BETWEEN ? AND ?", '2018-02-01','2018-02-28').pluck("t_user_info.F_id").uniq.size
